...
namespace Whisper
{
    ...
    public class WhisperManager : MonoBehaviour
    {
        ...
        private string modelPath = "Whisper/ggml-tiny.en-q8_0.bin";
        ...
        public async Task InitModel()
        {
            ...
            {
                string fileData = "";
                string fileName = Path.Combine(Application.streamingAssetsPath, modelPath);
                byte[] bytes = UnityEngine.Windows.File.ReadAllBytes(fileName);

                fileData = System.Text.Encoding.ASCII.GetString(bytes);

                LogUtils.Warning("Loading: "+ fileData + ", the original was: "+ fileName);
                _whisper = await WhisperWrapper.InitFromFileAsync(fileName);
